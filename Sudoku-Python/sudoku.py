...
def main():
	...
	pygame.display.set_caption("Sudoku Game by Ansh")
	BASICFONT = pygame.font.Font('freesansbold.ttf', 20)
	RESET_SURF, RESET_RECT = makeText(' Reset ', TEXTCOLOR, RED, WINDOWWIDTH-120, WINDOWHEIGHT-20)
	NEW_SURF, NEW_RECT = makeText(' New Game ', TEXTCOLOR, BOXCOLOR, WINDOWWIDTH-120, WINDOWHEIGHT-80)
	SOLVE_SURF, SOLVE_RECT = makeText(' Solve ', TEXTCOLOR, GREEN, WINDOWWIDTH-120, WINDOWHEIGHT-50)
	...
	while True:
			...
			if event.type == QUIT or (event.type==KEYUP and event.key == K_ESCAPE):
				...
				if (boxx, boxy) == (None, None):
					if SOLVE_RECT.collidepoint(mousex, mousey):
						...
						reset()
						MSG = True
						MSG_SURF, MSG_RECT = makeText('Solving Please Wait!!!', BLACK, BGCOLOR, WINDOWWIDTH//2-100, WINDOWHEIGHT-30)	
						solve()
		...
		if boxx!=None and boxy!=None:
			...
			if SOLVE_RECT.collidepoint(mousex, mousey):
				SOLVE_SURF, SOLVE_RECT = makeText(' Solve ', GREEN, TEXTCOLOR, WINDOWWIDTH-120, WINDOWHEIGHT-50)
				SOLVE_CHECK = 1
			elif RESET_RECT.collidepoint(mousex, mousey):
				RESET_SURF, RESET_RECT = makeText(' Reset ', RED, TEXTCOLOR, WINDOWWIDTH-120, WINDOWHEIGHT-20)
				RESET_CHECK = 1
			elif NEW_RECT.collidepoint(mousex, mousey):
				NEW_SURF, NEW_RECT = makeText(' New Game ', BOXCOLOR, TEXTCOLOR, WINDOWWIDTH-120, WINDOWHEIGHT-80)
				NEWGAME_CHECK = 1
